...
2. **Endpoints & EndpointSlices:**
   ...
   - When a client sends traffic to the Service's ClusterIP (`demo-dev-service:80`), CoreDNS resolves the service hostname to the ClusterIP, and kube-proxy (or IPVS/eBPF) load-balances the traffic across those healthy Pod IPs.
...
- In Kubernetes, when `spec.replicas` is omitted or null, the API server accepts the object and defaults `spec.replicas` to `1`:

  ```bash
  helm template brk ./charts/nginx-demo | kubectl apply --dry-run=server -n helm-lab -f -
  # deployment.apps/brk-deployment created (server dry run)
  ```

- **Why this matters:** Silent template omissions are dangerous. If your chart intended to run 3 or 5 replicas in production, a simple typo silently scales the application down to 1 replica with zero errors from Helm or Kubernetes! This is why production charts use the `required` function or schema validation (`values.schema.json`, covered in Lab 7).
