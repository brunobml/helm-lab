...
- The Kubernetes API server will reject the manifest with an unmarshaling error:

  ```text
  cannot patch "demo-dev-deployment" ... json: cannot unmarshal bool into Go struct field EnvVar.spec.template.spec.containers.env.value of type string
  ```

  *(Note that `helm template` does NOT catch this: it happily renders `value: false` as valid YAML! Only cluster submission or OpenAPI schema validation catches it).*
